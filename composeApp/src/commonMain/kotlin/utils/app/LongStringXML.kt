...
class LongStringXML() {
    ...
    @Deprecated("Replace By AboutSG3Content")
    ...
    fun AboutSG3Desc(appLanguage: Language.AppLanguage = Language.AppLanguageInstance): String{
        return when (appLanguage){
            Language.AppLanguage.ZH_HK -> "Stargazer 3 (星穹觀星者 3) 是由Coding Band開發的一款跨平台星鐵第三方助手應用，旨在協助開拓者的實用工具。\n\n我們期望能透過本次迭代，能為各位開拓者提供更多元化的平台選擇，優化以往在使用體驗上的不足，以及對貢獻者友善。\n\n由於開發人員人力有限，部分在上一版本的功能將暫時不納入初版。我們會盡力在未來版本重新編寫及推出相關功能。"
            Language.AppLanguage.ZH_CN -> "Stargazer 3 (星穹观星者 3) 是由Coding Band开发的一款跨平台星铁第三方助手应用，旨在协助开拓者的实用工具。\n\n我们期望能通过本次迭代，能为各位开拓者提供更多元化的平台选择，优化以往在使用体验上的不足，以及对贡献者友善。\n\n由于开发人员人力有限，部分在上一版本的功能将暂时不纳入初版。我们会尽力在未来版本重新编写及推出相关功能。"
            Language.AppLanguage.PT -> "Stargazer 3 (星穹观星者 3) é um aplicativo assistente Star Rail de terceiros multiplataforma desenvolvido pela Coding Band, com o objetivo de fornecer ferramentas práticas para os exploradores.\n\nEsperamos que, através desta iteração, possamos fornecer aos exploradores uma escolha de plataforma mais diversificada, otimizar as deficiências na experiência de uso anterior e ser amigável com os contribuintes.\n\nDevido à limitação de recursos humanos dos desenvolvedores, algumas funções da versão anterior não serão incluídas na versão inicial. Faremos o possível para reescrever e lançar as funções relevantes nas futuras versões."
            else -> "Stargazer 3 (星穹观星者 3) is a cross-platform third-party Star Rail assistant application developed by Coding Band, aiming to provide practical tools for explorers.\n\nWe hope that through this iteration, we can provide explorers with a more diversified platform choice, optimize the deficiencies in the previous user experience, and be friendly to contributors.\n\nDue to the limited human resources of the developers, some functions from the previous version will temporarily not be included in the initial version. We will do our best to rewrite and launch the relevant functions in future versions."
        }
    }

    fun AboutSG3CodingBand(appLanguage: Language.AppLanguage = Language.AppLanguageInstance) : String{
        return when (appLanguage){
            Language.AppLanguage.ZH_HK -> "Coding Band 是一個專注於開發具實用性、鑽研軟體内藝術和趣味性的開發團隊。\n\n參與了本次開發的成員："
            Language.AppLanguage.ZH_CN -> "Coding Band 是一个专注于开发具实用性、钻研软件内艺术和趣味性的开发团队。\n\n参与了本次开发的成员："
            Language.AppLanguage.PT -> "Coding Band é uma equipe de desenvolvimento que se concentra no desenvolvimento da praticidade, mergulhando na arte e na diversão do software.\n\nMembros que participaram do desenvolvimento desta vez:"
            else -> "Coding Band is a development team that focuses on developing practicality, delving into the art and fun of software.\n\nMembers who participated in the development this time:"
        }
    }

    fun AboutSG3SpecialThanks(appLanguage: Language.AppLanguage = Language.AppLanguageInstance) : String{
        return when (appLanguage){
            Language.AppLanguage.ZH_HK -> "特別感謝參與本次封測人員："
            Language.AppLanguage.ZH_CN -> "特别感谢参与本次封测人员："
            Language.AppLanguage.PT -> "Agradecimentos especiais aos testadores desta vez:"
            else -> "Special thanks to the testers this time:"
        }
    }

    fun AboutSG3Disclaimer(appLanguage: Language.AppLanguage = Language.AppLanguageInstance) : String {
        return when (appLanguage){
            Language.AppLanguage.ZH_CN -> "请注意：本项目及应用程序并不属于 miHoYo Co., Ltd. 且并未获得其认可。Stargazer 3（星穹观星者 3）仅为一款由粉丝自行开发之数据提供 App，本应用程序所提供的数据只作参考用途。 不会对其所载任何数据的准确性及完整作任何程度保证。\n\n「Stargazer (星穹观星者)」项目内使用之各素材版权归该版权所有者拥有。\n「Stargazer (星穹观星者)」项目原始码著作权归 Coding Band 所有。"
            Language.AppLanguage.ZH_HK -> "請注意：本專案及應用程式並不屬於 miHoYo Co., Ltd. 且並未獲得其認可。Stargazer 3（星穹觀星者 3）僅為一款由粉絲自行開發之數據提供 App，本應用程式所提供的資料只作參考用途。 不會對其所載任何資料的準確性及完整作任何程度保證。\n\n「Stargazer (星穹觀星者)」專案内使用之各素材版權歸該版權所有者擁有。\n「Stargazer (星穹觀星者)」專案原始碼著作權歸 Coding Band 所有。"
            Language.AppLanguage.PT -> "Por favor, note: Este projeto e aplicativo não pertencem à miHoYo Co., Ltd. Stargazer 3 é apenas um projeto que fornece dados desenvolvidos por fãs. As informações fornecidas neste aplicativo são apenas para referência. Stargazer 3 não garante a exatidão e integridade de qualquer informação contida em qualquer grau.\n\nOs direitos autorais de cada material usado no projeto 'Stargazer 3' pertencem ao detentor dos direitos autorais.\nOs direitos autorais do código do projeto 'Stargazer 3' pertencem à Coding Band."
            else -> "Please note: This project and application do not belong to miHoYo Co., Ltd. and have not been approved. Stargazer 3 is only a project that provides data developed by fans. The information provided in this app is for reference only. Stargazer 3 will not guarantee the accuracy and completeness of any information it contains to any degree.\n\nThe copyright of each material used in the 'Stargazer 3' project belongs to the copyright holder.\nThe copyright of the 'Stargazer 3' project code belongs to Coding Band."
        }
    }
