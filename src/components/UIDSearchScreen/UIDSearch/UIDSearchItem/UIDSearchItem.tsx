    ...
    return (
        ...
        <View className="w-full h-[60px] px-3 bg-[#F3F9FF40] rounded-[10px]" style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
            <View style={{ flexDirection: "row", gap: 9 }}>
                <Image
                    // @ts-ignore
                    source={AvatarIcon[user?.player?.avatar?.icon?.match(/\d+/g).join("")] || AvatarIcon}
                    className="w-10 h-10 rounded-full bg-[#CAB89E]" />
                <View>
                    <Text className="text-text text-[16px] font-[HY65] leading-5">{uuid}</Text>
                    <Text className="text-text text-[12px] font-[HY65] leading-5">{user?.player?.nickname} ·
