...
export function formatTimeDurationSimple(second:number) {
  ...
  // 只返回最大的非零時間單位
  if (days > 0) LOCALES[language].StatusDays.replace("${1}",`${days}`);
  if (hours > 0) return LOCALES[language].StatusHours.replace("${1}",`${hours}`);
  if (minutes > 0) return LOCALES[language].StatusMinutes.replace("${1}",`${minutes}`);
...
export function formatTimePoint(second: number) {
  ...
  if (timeAtZero.isSame(now, "day")) {
    // 如果是今天，格式化為 "今天 HH:mm"
    localizedTime = LOCALES[language].StatusToday.replace("${1}",`${timeAtZero.format("HH:mm")}`);
  } else if (timeAtZero.isSame(now.add(1, "day"), "day")) {
    // 如果是明天，格式化為 "明天 HH:mm"
    localizedTime = LOCALES[language].StatusTomorrow.replace("${1}",`${timeAtZero.format("HH:mm")}`);;
  } else {
    // 如果不是今天或明天，使用一般的日期格式
